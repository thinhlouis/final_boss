import "./MovieDetail.css";
import MoviesContext from "../../context/MovieContext/MoviesContext";
import CustomDetails from "./CustomDetails/CustomDetails";
import video_not_found from "../../assets/video-not-found.png";

import React from "react";
import { useState, useEffect, useContext } from "react";
import { useParams, Link } from "react-router-dom";
import { AiFillHome } from "react-icons/ai";
import { BarLoader } from "react-spinners";
import axios from "axios";

export default function MovieDetail() {
  const [datas, setDatas] = useState({
    movie: {},
    episodes: {},
  });

  const [notification, setNotification] = useState("");
  const [category, setCatagory] = useState([]);
  const [actor, setActor] = useState("");

  const [loading, setLoading] = useState(true);

  const { movies } = useContext(MoviesContext);
  const { slug_movie } = useParams();

  useEffect(() => {
    const fetchMovie = async () => {
      try {
        const response = await axios.get(
          `https://phimapi.com/phim/${slug_movie}`
        );

        if (!response.data.status) {
          setNotification(response.data.msg);
          return;
        }

        const { movie, episodes } = response.data;

        setDatas({
          movie: movie,
          episodes: episodes,
        });

        const actor = movie?.actor.join(", ") || "Chưa cập nhật";
        const categorys = movie?.category.map((c) => c.name);

        setActor(actor);
        setCatagory(categorys);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
      }
    };

    fetchMovie();
  }, [movies, slug_movie]);

  const openPlayer = (index, name, eps) => {
    const url = window.location.origin;
    window.open(
      `${url}/ksc/player/${name}/${eps}`,
      "_blank",
      "noopener,noreferrer"
    );

    localStorage.setItem("idxsv", index);
  };

  const {
    movie: {
      slug,
      origin_name,
      name,
      episode_current,
      episode_total,
      time,
      year,
      quality,
      lang,
      director,
      country,
      poster_url,
      content,
    },
    episodes,
  } = datas;

  if (loading) {
    return (
      <div
        style={{
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          flexDirection: "column",
          gap: "5px",
          height: "50vh",
        }}
      >
        <p>Đang loading movie...</p>
        <BarLoader color="#657e1f" height={5} width={150} />
      </div>
    );
  }

  return (
    <div className="movie-detail-container">
      {notification ? (
        <div>
          <p style={{ color: "#F81101", marginBottom: "1rem" }}>
            {notification} <Link to="/ksc/movies">Quay lại</Link>
          </p>

          <img
            src={video_not_found}
            alt="movie-not-found"
            style={{ width: "500px" }}
          />
        </div>
      ) : (
        <>
          <nav className="nav-header-movie">
            <ul>
              <li className="home">
                <Link to="/ksc/movies" title="Back movies page">
                  <AiFillHome />
                </Link>
              </li>
              <li className="link-movie">
                <Link to="#" title={slug}>
                  {name}
                  <span> - {origin_name}</span>
                </Link>
              </li>
            </ul>
          </nav>
          <div className="movie-detail-box">
            <div className="movie-detail-box_item item-border">
              <div className="movie-box-item-img">
                <img src={poster_url} alt={name} />
              </div>
              <div className="movie-box-item-detail">
                <div className="header-movie root_flex_column gap_025 item_start">
                  <h2 className="header-movie-name">{name}</h2>
                  <p className="header-movie-origin-name">( {origin_name} )</p>
                </div>
                <p className="detail-of-movie">
                  <span className="span-detail">Tình trạng</span>
                  <span className="span-info">{episode_current}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Số tập</span>
                  <span className="span-info">{episode_total}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Thời lượng</span>
                  <span className="span-info">{time}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Năm phát hành </span>
                  <span className="span-info">{year}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Chất lượng</span>
                  <span className="span-info">{quality}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Ngôn ngữ</span>
                  <span className="span-info">{lang}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Đạo diễn</span>
                  <span className="span-info">{director[0]}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Diễn viên</span>
                  <span className="span-info">{actor}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Thể loại</span>
                  <span className="span-info">{category.join(",")}</span>
                </p>
                <p className="detail-of-movie">
                  <span className="span-detail">Quốc gia</span>
                  <span className="span-info">{country[0]?.name}</span>
                </p>
              </div>
            </div>
            <div className="movie-detail-box_item">
              <CustomDetails title="Nội dung Movie">
                <p>{content}</p>
              </CustomDetails>
            </div>
            <div className="movie-detail-box_item">
              <CustomDetails title="Xem Movie">
                {episodes?.length > 0 && (
                  <>
                    <div className="title-name-sever">
                      {episodes?.map((eps, idx) => (
                        <div
                          key={idx}
                          className="root_flex_column item_start gap_05"
                        >
                          <h3>SERVER:</h3>
                          <span className="server_name">
                            {eps?.server_name}
                          </span>
                          <div className="item-episode">
                            {eps?.server_data?.map((ep, index) => (
                              <div key={index} className="btn-eps">
                                {eps?.server_data?.length === 1 && (
                                  <button
                                    onClick={() =>
                                      openPlayer(idx, slug, ep.slug)
                                    }
                                    title={ep.filename}
                                  >
                                    <Link
                                      to={`/ksc/player/${slug}/${ep.slug}`}
                                      onClick={(e) => e.preventDefault()}
                                    >
                                      {ep?.name.split("Tập ")[1] ?? ep.name}
                                    </Link>
                                  </button>
                                )}
                                {index <=
                                  Math.floor(
                                    eps?.server_data?.length * 0.4 - 1
                                  ) && (
                                  <button
                                    onClick={() =>
                                      openPlayer(idx, slug, ep.slug)
                                    }
                                    title={ep.filename}
                                  >
                                    <Link
                                      to={`/ksc/player/${slug}/${ep.slug}`}
                                      onClick={(e) => e.preventDefault()}
                                    >
                                      {ep?.name.split("Tập ")[1] ?? ep.name}
                                    </Link>
                                  </button>
                                )}
                                {index >
                                  Math.floor(
                                    eps?.server_data?.length * 0.4 - 1
                                  ) &&
                                  eps?.server_data?.length > 1 && (
                                    <div
                                      className="box-btn-vip root_flex_column"
                                      onClick={() =>
                                        openPlayer(idx, slug, ep.slug)
                                      }
                                      title={ep.filename}
                                    >
                                      <button className="btn-vip-eps">
                                        <Link
                                          to={`/ksc/player/${slug}/${ep.slug}`}
                                          onClick={(e) => e.preventDefault()}
                                        >
                                          {ep?.name.split("Tập ")[1] ?? ep.name}
                                        </Link>
                                      </button>

                                      <small className="vip-eps">VIP</small>
                                    </div>
                                  )}
                              </div>
                            ))}
                          </div>
                        </div>
                      ))}
                    </div>
                  </>
                )}
              </CustomDetails>
            </div>
          </div>
        </>
      )}
    </div>
  );
}
